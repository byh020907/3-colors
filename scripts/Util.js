    ...
    while ((currentPoint.x != finalPoint.x || currentPoint.y != finalPoint.y) && currentPoint.x >= 0 && currentPoint.y >= 0 && currentPoint.x < mapData[0].length && currentPoint.y < mapData.length) {
        ...
        for (var i = 0; i < tops.length; i++) {
            if (tops[i] == 0) {
                ...
                distances.push({
                    direction: 0,
                    distance: Math.sqrt(Math.pow(finalPoint.x - (currentPoint.x - 1), 2) + Math.pow(finalPoint.y - currentPoint.y, 2))
                });
            } else if (tops[i] == 1) {
                distances.push({
                    direction: 1,
                    distance: Math.sqrt(Math.pow(finalPoint.x - currentPoint.x, 2) + Math.pow(finalPoint.y - (currentPoint.y - 1), 2))
                });
            } else if (tops[i] == 2) {
                distances.push({
                    direction: 2,
